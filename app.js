...
Router.register('/dashboard', () => {
    ...
    // Setup YouTube sync button
    const syncBtn = document.getElementById('syncYouTube');
    if (syncBtn) {
        syncBtn.onclick = handleYouTubeSync;
    }
});
...
function setupYouTubeAdmin() {
    ...
    if (isYouTubeConnected()) {
        ...
        statusDiv.innerHTML = '<p class="success">✓ Connected</p>';
        connectBtn.onclick = () => {
            disconnectYouTube();
...
async function handleYouTubeSync() {
    ...
    try {
        const videos = await getTodayWatchHistory();
        
        if (videos.length === 0) {
            alert('No videos watched today found.');
            return;
        }
        
        displayYouTubeVideos(videos);
        openModal('youtubeImportModal');
    } catch (error) {
        console.error('YouTube sync failed:', error);
        alert('Failed to fetch YouTube watch history. Please try again.');
    }
}
...
function displayYouTubeVideos(videos) {
    ...
    container.innerHTML = videos.map((video, index) => `
        <div class="youtube-video-item">
            <input type="checkbox" id="yt-${index}" class="youtube-video-checkbox" data-index="${index}">
            <label for="yt-${index}" class="youtube-video-label">
                ...
                <div class="youtube-video-info">
                    ...
                    <span class="youtube-duration">${video.duration || '?'} minutes</span>
                </div>
            </label>
        </div>
    `).join('');
    
    // Store videos data for import
    window.youtubeVideosData = videos;
    
    // Setup import button
    const importBtn = document.getElementById('importSelectedVideos');
    if (importBtn) {
        importBtn.onclick = () => {
            const checkboxes = document.querySelectorAll('.youtube-video-checkbox:checked');
            const selectedVideos = Array.from(checkboxes).map(cb => {
                const index = parseInt(cb.dataset.index);
                return window.youtubeVideosData[index];
            });
            ...
            importYouTubeVideos(selectedVideos);
        };
    }
}

function importYouTubeVideos(videos) {
    const today = getDateString(new Date());
    ...
    videos.forEach(video => {
        const entry = {
            name: video.title,
            type: 'Video',
            duration: video.duration || 10, // Default to 10 if unknown
            date: today
        };
        
        addMedia(entry);
        addOrUpdateSource(video.title, 'Video', video.thumbnail);
    });
    
    closeModal('youtubeImportModal');
    render();
